...
from pydub import AudioSegment  # Assuming this is needed for the run function
                ...
                while True:
                    ...
                    if has_activity:
                        ...
                        if audio_activity_detected and not self.recording_started:
                            ...
                            # Send full file path to the transcription handler (first run)
                            self.meeting_analyzer.run(self.full_recording_path, 0, 0)
                    ...
                    if self.recording_started:
                        ...
                        if time.time() - chunk_start_time >= self.chunk_interval:
                            ...
                            with wave.open(chunk_path, 'wb') as chunk_file:
                                ...
                                chunk_file.setframerate(16000)z
                                chunk_file.writeframes(buffer)
                            buffer = bytearray()
                            chunk_start_time = time.time()

                            # Calculate the start and end times for the chunk
                            start_time = chunk_start_time - self.meeting_start_time.timestamp()
                            end_time = time.time() - self.meeting_start_time.timestamp()

                            # Send chunk start and end times to the transcription handler
                            self.meeting_analyzer.run(chunk_path, start_time, end_time)
